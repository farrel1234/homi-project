...
fun FormSuratKeteranganUsahaScreen(
    ...
    LaunchedEffect(isSelf, profileData) {
        ...
        if (isSelf) {
            ...
            tanggalLahir = ""
            jenisKelamin = ""
            // RT tetap ambil dari profile login warga
            rt = p.residentProfile?.rt ?: ""
            rw = p.residentProfile?.rw ?: ""
            namaRt = p.residentProfile?.namaRt ?: ""
        }
    ...
    val canSubmit = nama.isNotBlank() && nik.isNotBlank() && alamat.isNotBlank() && 
                    ...
                    namaUsaha.isNotBlank() && jenisUsaha.isNotBlank() && 
                    alamatUsaha.isNotBlank() && lamaUsaha.isNotBlank() && keperluan.isNotBlank()
    ...
    Column(
        ...
        Column(
            ...
            Text(
                text = "Lengkapi data untuk pembuatan Surat Keterangan Usaha\nlegalitas usaha warga Hawaii Garden.",
        ...
        Surface(
            ...
            Column(
                ...
                HomiFormField("Nama Lengkap", "Misal: Budi Santoso", nama, { nama = it }, enabled = !isSelf || nama.isBlank())
                Spacer(Modifier.height(16.dp))
                HomiFormField("NIK", "Masukkan 16 digit NIK", nik, { nik = it.filter { c -> c.isDigit() }.take(16) }, KeyboardType.Number, enabled = !isSelf)
                Spacer(Modifier.height(16.dp))
                ...
                Spacer(Modifier.height(16.dp))
                HomiFormField("Alamat Lengkap", "Alamat sesuai KTP/KK", alamat, { alamat = it }, singleLine = false, minLines = 2, enabled = false)
                ...
                HomiFormField("Nama Usaha", "Misal: Warung Sembako Berkah", namaUsaha, { namaUsaha = it })
                Spacer(Modifier.height(16.dp))
                HomiFormField("Jenis Usaha", "Contoh: Kuliner / Sembako / Jasa", jenisUsaha, { jenisUsaha = it })
                Spacer(Modifier.height(16.dp))
                HomiFormField("Alamat Usaha", "Lokasi atau alamat tempat usaha berdiri", alamatUsaha, { alamatUsaha = it }, singleLine = false, minLines = 2)
                Spacer(Modifier.height(16.dp))
                HomiFormField("Lama Usaha", "Misal: 2 Tahun / 6 Bulan", lamaUsaha, { lamaUsaha = it })
                Spacer(Modifier.height(16.dp))
                HomiFormField("Keperluan", "Contoh: Pengajuan Kredit Bank / Izin RT", keperluan, { keperluan = it }, singleLine = false, minLines = 2)
                ...
                Button(
                    onClick = {
                        val payload = mutableMapOf(
                            ...
                            "nama_usaha" to namaUsaha.trim(),
                            "jenis_usaha" to jenisUsaha.trim(),
                            "alamat_usaha" to alamatUsaha.trim(),
                            "lama_usaha" to lamaUsaha.trim(),
                ...
                Text(
                    text = if (canSubmit) "✓ Data usaha sudah lengkap." else "✕ Harap lengkapi semua data.",
                    fontFamily = PoppinsReg,
                    fontSize = 12.sp,
...
@Composable
private fun PreviewFormSuratKeteranganUsahaPremium() {
    MaterialTheme {
        // Mock repository for preview
        val mockRepo = AccountRepository(ApiClient.getApiMock())
        FormSuratKeteranganUsahaScreen(accountRepo = mockRepo)
    }
}
