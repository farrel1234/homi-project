...
<div class="space-y-8 py-4">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div class="space-y-1">
            <h1 class="text-3xl font-black tracking-tight text-slate-900 uppercase">Daftar Pengajuan</h1>
            <p class="text-slate-500 font-medium">Kelola surat menyurat dan permohonan layanan warga</p>
        </div>
    ...
    <div class="homi-card p-6 overflow-visible border-none shadow-xl shadow-slate-200/50">
        <form action="{{ route('service-requests.index') }}" method="GET" class="flex flex-col lg:flex-row gap-4 items-center">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100 italic">
                        <th class="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Warga & Kontak</th>
                        <th class="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Jenis Pengajuan</th>
                        <th class="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Subjek / Detail</th>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center font-black text-slate-400 group-hover:bg-white group-hover:text-[var(--homi-blue)] transition-colors">
                            ...
                            <td class="px-8 py-6">
                                <div class="font-black text-slate-700 leading-none mb-1">{{ $req->type->name ?? 'Layanan Umum' }}</div>
                                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.1em]">ID: #{{ str_pad($req->id, 5, '0', STR_PAD_LEFT) }}</div>
                            </td>
                            <td class="px-8 py-6">
                                <div class="max-w-xs truncate font-medium text-slate-600 italic">
                            ...
                            <td class="px-8 py-6">
                                <div class="font-bold text-slate-900 leading-none mb-1">{{ optional($req->created_at)->format('d M Y') }}</div>
