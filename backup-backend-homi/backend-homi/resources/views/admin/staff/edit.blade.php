    ...
    <div class="homi-card p-0 overflow-hidden shadow-2xl shadow-slate-200 border-none">
        ...
        <div class="bg-slate-900 p-8 text-white relative">
            ...
            <div class="relative z-10">
                <h1 class="text-2xl font-black tracking-tight mb-2 uppercase">Edit Profil Staff</h1>
                <p class="text-slate-400 text-sm font-medium">Perbarui data atau role pengelola</p>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.staff.update', $item->id) }}" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $item->name) }}" placeholder="Contoh: Admin" 
                           class="homi-input font-bold" required>
                </div>

                <div class="space-y-2">
                    <label class="homi-label text-slate-900">Nama Lengkap</label>
                    <input type="text" name="full_name" value="{{ old('full_name', $item->full_name) }}" placeholder="Masukkan nama lengkap..." 
                           class="homi-input font-bold">
                </div>

                <div class="space-y-2">
                    <label class="homi-label text-slate-900">Email <span class="text-rose-500">*</span></label>
                    <input type="email" name="email" value="{{ old('email', $item->email) }}" placeholder="staff@example.com" 
                           class="homi-input font-bold" required>
                </div>

                <div class="space-y-2">
                    <label class="homi-label text-slate-900">Username <span class="text-rose-500">*</span></label>
                    <input type="text" name="username" value="{{ old('username', $item->username) }}" placeholder="username" 
                           class="homi-input font-bold" required>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select name="role" class="homi-input font-bold" required>
                        <option value="admin" @selected(old('role', $item->role) == 'admin')>Admin Toko/Perumahan</option>
                        <option value="superadmin" @selected(old('role', $item->role) == 'superadmin')>Owner / Super Admin</option>
                    </select>
                </div>
                ...
                <div class="space-y-2 col-span-1 md:col-span-2" id="tenant-select-container">
                    ...
                    <select name="tenant_id" class="homi-input font-bold">
                        ...
                        @foreach($tenants as $ten)
                            <option value="{{ $ten->id }}" @selected(old('tenant_id', $item->tenant_id) == $ten->id)>{{ $ten->name }} ({{ $ten->code }})</option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-50">
                    <div class="md:col-span-2">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Ganti Password (Kosongkan jika tidak ingin mengubah)</p>
                    </div>
                    <div class="space-y-2">
                        <label class="homi-label text-slate-900">Password Baru</label>
                        <input type="password" name="password" placeholder="••••••••" 
                               class="homi-input font-bold">
                    </div>

                    <div class="space-y-2">
                        <label class="homi-label text-slate-900">Konfirmasi Password Baru</label>
