    ...
    <div class="homi-card p-0 overflow-hidden shadow-2xl shadow-slate-200">
        ...
        <div class="bg-slate-900 p-8 text-white relative">
            ...
            <div class="relative z-10">
                <h1 class="text-2xl font-black tracking-tight mb-2 uppercase">Buat Pengumuman</h1>
                <p class="text-slate-400 text-sm font-medium">Sampaikan informasi penting kepada seluruh warga {{ session('tenant_name', 'Homi') }}</p>
            </div>
        </div>

        <form action="{{ route('announcements.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf

            {{-- Row 1: Judul --}}
            <div class="space-y-2">
                <label class="homi-label text-slate-900">Judul Pengumuman</label>
                <input type="text" name="title" value="{{ old('title') }}" placeholder="Ketikkan judul yang menarik..." 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <select name="category" class="homi-input font-bold">
                        <option value="Informasi Umum" @selected(old('category') == 'Informasi Umum')>Informasi Umum</option>
                        ...
                        <option value="Pembangunan" @selected(old('category') == 'Pembangunan')>Pembangunan</option>
            ...
            <div class="flex flex-wrap items-center gap-4 bg-slate-50 p-6 rounded-[2rem] border border-slate-100 shadow-inner">
                ...
                <label class="flex items-center gap-3 cursor-pointer group">
                    ...
                    <div class="flex flex-col">
                        <span class="text-sm font-black text-slate-700 uppercase tracking-widest leading-none">Publikasikan</span>
                        <span class="text-[10px] text-slate-400 font-bold uppercase mt-1">Langsung tampil di aplikasi</span>
                    </div>
                </label>
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="rounded-3xl overflow-hidden border border-slate-200 shadow-inner">
                    <textarea id="editor" name="body" rows="8">{{ old('body') }}</textarea>
                </div>
            </div>
